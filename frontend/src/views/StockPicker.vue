...
async function loadNotificationSettings() {
  try {
    const settings = await stockPickerApi.getNotificationSettings()
    pushEnabled.value = settings.weekly_picker_push
  } catch {
    // ignore
  }
}
...
async function saveNotificationSettings() {
  try {
    await stockPickerApi.updateNotificationSettings({ weekly_picker_push: pushEnabled.value })
    showToast(t('stockPicker.settingsSaved'))
    showSettings.value = false
  } catch (e: any) {
    showToast(t('stockPicker.settingsError') + ': ' + e.message, 'error')
  }
}
  ...
  <div class="picker-page">
    ...
    <section v-if="weeklySummary?.has_new_weekly" class="weekly-banner card">
      <div class="weekly-header">
        <div>
          <h3 class="weekly-title">{{ t('stockPicker.weeklyTitle') }}</h3>
          <p class="weekly-meta">
            {{ t('stockPicker.generatedAt') }}: {{ formatDate(weeklySummary.generated_at) }} ·
        ...
        <template v-else>
          <div class="editor-toolbar">
            ...
            <div class="toolbar-actions">
              ...
              <button class="btn btn--ghost" @click="showSettings = true">{{ t('stockPicker.settings') }}</button>
            </div>
          </div>
    ...
    <div v-if="showSettings" class="modal-overlay" @click.self="showSettings = false">
      <div class="modal">
        <h3 class="modal-title">{{ t('stockPicker.settingsTitle') }}</h3>
        <label class="toggle-row">
          <input v-model="pushEnabled" type="checkbox" />
          <span>{{ t('stockPicker.weeklyPushLabel') }}</span>
        </label>
        <div class="modal-actions">
          <button class="btn btn--primary" @click="saveNotificationSettings">{{ t('common.save') }}</button>
...
.picker-workshop {
  display: flex;
  height: calc(100vh - var(--header-height) - (var(--space-3xl) * 2) - 140px);
  background: var(--bg-surface);
  border: 1px solid var(--border-subtle);
  border-radius: var(--radius-lg);
